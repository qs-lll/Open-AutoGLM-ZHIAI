...
object Prompts {
    ...
    // 获取格式化的当前日期（yyyy年MM月dd日 星期X）
    private val formattedDate: String
        get() {
            val today = LocalDate.now()
            val weekday = weekdayNames[today.dayOfWeek.value - 1] // DayOfWeek.MONDAY = 1，对应数组索引0
            val dateFormatter = DateTimeFormatter.ofPattern("yyyy年MM月dd日", Locale.CHINA)
            return "${today.format(dateFormatter)} $weekday"
        }
    ...
    """.trimIndent()
    fun getSystemPrompt(lang: String = "cn"): String {
        return if (lang == "en") SYSTEM_PROMPT else SYSTEM_PROMPT
    }
}